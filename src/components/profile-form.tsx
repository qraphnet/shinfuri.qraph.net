import { kishuForeignLangList, shohuForeignLangList, languageCodeMap, karuiJa, classNumList, LanguageOption } from 'shinfuri/lib/type-utils.js';

import { FC, Suspense, FormEventHandler } from 'react';
import { useRecoilValue, useRecoilCallback } from 'recoil';

import { field } from '../dataflow/util.js';
import { Profile, profileState } from '../dataflow/profile/index.ts';
import { profileForm, validated, isValid } from '../dataflow/profile/form.js';
...
export const ProfileForm: FC<{ profile?: Profile }> = ({ profile }) => {
  ...
  return <Form onSubmit={submitHandler} className='profile-form'>
    <fieldset>
      <legend>所属</legend>
      <label>科類：<select name="karui" defaultValue={ profile?.karui }>
        { Object.entries(karuiJa).map(([karui, text]) => <option key={karui} value={karui}>{text}</option>) }
      </select></label>
      <label>組：<select name="classNum" defaultValue={ profile?.classNum }>
    ...
    <fieldset>
      ...
      <label><input type="checkbox" name="learned" defaultChecked={ profile?.langOption.secondForeignLanguage.learned } />既修相当</label>
    </fieldset>
    <fieldset>
      <legend>留年・降年</legend>
      <label>最後にしたのは：<select name="kind" defaultValue={ profile?.lastRepetition?.kind }>
        <option>したことない</option>
        <option value="留年">留年</option>
        <option value="降年">降年</option>
      </select></label>
      <label>それが確定した年度：<input type="number" name="year" defaultValue={ profile?.lastRepetition?.year } min={2000} step={1} /></label>
    </fieldset>
    <Suspense>
      <Submit />
    </Suspense>
...
const SecondLangOption: FC = () => {
  ...
  return (!learned ? shohuForeignLangList : kishuForeignLangList).map(l => <option key={l} value={l}>{languageCodeMap[l]}</option>);
};
