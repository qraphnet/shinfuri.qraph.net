...
import {alteredState, departmentsForm, inputDepartments, selectedDepartments} from '../dataflow/department-selected.js';
...
const format = (points: [Rational, Rational]) => {
  ...
  const [l, u] = points.map(p => "" + (Math.round(p.toNumber() * d) / d));
...
const DepartmentSelectInternal: FC<{ tree: DepartmentTree; selected: Set<Department> }> = ({ tree, selected }) => {
  return <div>
    { Object.entries(tree).map(([division, value]) => 'string' === typeof value
      ? <label key={value}><input type="checkbox" name={value} defaultChecked={selected.has(value)}/>{ division }</label>
      : <details key={division}><summary>{ division }</summary><DepartmentSelectInternal tree={value} selected={selected} /></details>
    ) }
  </div>;
};
