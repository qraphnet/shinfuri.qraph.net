...
import {isUnscoredGrade, scoredGrade, unscoredGrade, gradeRange} from "shinfuri/lib/report.js";
...
export const ReportForm: FC<JSX.IntrinsicElements['form']> = props => {
  ...
  const list = useMemo(() => Object.entries(getCourseCodeMap()).filter(([code]) => scope == null || scope.match(code as any)), [scope]);

  const defaultTitle = defaultReport && (dr?.courseTitle ?? getTitle(defaultReport.course.code));
  return <Form {...props}>
    <fieldset><legend>科目</legend>
...
const PointInput: FC<{ defaultValue?: number | [number, number] }> = ({ defaultValue: point }) => {
  ...
  return <>
    <label><input type="checkbox" checked={chickenbone && !disabled} onChange={e => setChickenbone(e.target.checked)} disabled={disabled}/>皮算用</label>
    <label>評点：{
      chickenbone
