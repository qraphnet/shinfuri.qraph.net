...
const allocate = <T,>(tree: CourseTree, values: T[], mapper: (v: T) => CourseCode | Scope, counter: (v: T) => number): AllocationTree<T> => {
  ...
  const sub = Array.from(map).map(([node, values]) =>
    ({ ...node, ...(node.sub == null ? { count: values.map(counter).reduce((a,c)=>a+c,0), empty: values.length === 0 ? 1 : 0, allocated: values, sub: [] } : allocate(node.sub, values, mapper, counter)) })
  );
  return { count, empty: sub.reduce((a, c) => a + c.empty, 0), allocated: rest, sub };
};


type Th = FC<{ colSpan?: number; rowSpan?: number; } & ({ name: string; scope: Scope } | { name?: undefined; scope?: undefined })>;
...
export const Rows = <T,>(props: RowsProps<T>) => {
  ...
  return Array.from(rowDataGenerator(allocationTree)).map(({ ths, value }, i) => <Row key={ths.map(v=>v.name??"").join('/')+'/'+i} value={value}>
    { ths.map((th, i) => th.i == 0 && <Th key={i} colSpan={i == ths.length - 1 ? sideWidth - i : 1} {...th}/>) }
  </Row>);
};

type ThData = { name: string; scope: Scope; rowSpan: number; i: number } | { name?: undefined; scope?: undefined; rowSpan: number; i: number };
